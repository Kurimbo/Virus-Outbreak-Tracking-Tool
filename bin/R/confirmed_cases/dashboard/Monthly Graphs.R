...
all_data <- all_data %>% filter(Year %in% c(2020:2023)) %>% mutate(State = "Texas")  # Filter to selected years only
  ...
  sidebarLayout(
    ...
    mainPanel(
      conditionalPanel(
        ...
        #tags$h3("No data available for the selected filters.", style = "color: red;")
      ),
